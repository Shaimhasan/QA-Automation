Feature: Basic Order Entry - Delivery Credit Card  - with Make Line Only
  This script is to validate Basic Order Entry - Delivery Credit Card  - with Make Line Only

  @issue=1619
  @Basic_Order_Entry_Dine_In_Delivery_Credit_Card_With_Make_Line_Only @RegressionSuite @BOE @BOE_Make_Line_Only @BOE_Make_Line_Only_Delivery
  Scenario: Basic_Order_Entry_Dine_In_Delivery_Credit_Card_With_Make_Line_Only_Testcase
    ...
    And the user validates Exact expected value "Compare_Strings" that the "table" element is "Equal To" "Credit Card" at the "OrderListPage" page based on datadictionary "#(order_Number)" and xpath1 "#(orderIdXpath)" and xpath2 "']//parent::td//following-sibling::td)[9]//div[text()='Cash']" "validate_ID_Number" "HardStopOnFailure"

Feature: Basic Order Entry - Delivery Credit Card  - with Make Line and Cut and Wrap enabled
  ...
  @issue=1654
  @Basic_Order_Entry_Dine_In_Delivery_Credit_Card_With_MC_Enabled @RegressionSuite @Delivery @BOE_Delivery @BOE @BOE_Make_Line_And_Cut_And_Wrap @BOE_Make_Line_And_Cut_And_Wrap_Delivery
  Scenario: Basic_Order_Entry_Dine_In_Delivery_Credit_Card_With_MC_Enabled_Testcase
    ...
    And the user clicks the "printerOkBtn" element at the "CommonPage" page If Present If Present
    #Comment: user click on Driver o Dispatch page
    And the user clicks the "bobTheDriver" element at the "DispatchPage" page
    #Comment: The user wait until page is loading
    And the user waits for the dom to load

Feature: Online Order - Item with Make Line and Cut and Wrap enabled - with all lines enabled - Guest
  This script is to validate Online Order - Item with Make Line and Cut and Wrap enabled - with all lines enabled - Guest

  @issue=3051
  @OT_By_Order_Submitted_Btn_Item_With_MC_Enabled_With_ALE_Existing_User @RegressionSuite @Submitted @Existing_User
  Scenario: OT_By_Order_Submitted_Btn_Item_With_MC_Enabled_With_ALE_Existing_User_Testcase
    ...
    #/**  Here we are validation the order status changed
    ...
    And the user waits for the "trackYourOrderTxt" element to be "VISIBLE" on the "OrderTrack" page
    #Comment: the user validate the element is present
    And the user validates the given "#(trackerCompletedMaking)" element is present at Page within given custom time interval "120" seconds
    #Comment: the user validate the element is present

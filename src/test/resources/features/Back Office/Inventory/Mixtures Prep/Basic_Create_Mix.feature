Feature: Basic Create Mix
  This script is to validate Create Mix

  @Basic_Create_Mix @RegressionSuite @BO_Mixtures_Prep @Inventory @Back_Office
  Scenario: Basic_Create_Mix_Testcase
    ...
    #Comment: the user store the text on data dictionary
    And store the displayed text of the "making" element at the "EditMixturesPrepPage" page into the data dictionary with key "making_value2"
    #Comment: user validate Name US Value
    And the user validates the data dictionary value of "#(making_value1)" is "Equal To" data dictionary value of "#(making_value2)" "validate_invoiceNum_value" "HardStopOnFailure"
    ...
    #Comment append value with dictionary
    And the user validates and append at trailing any value "']//preceding-sibling::td[text()='Add']" with data dictionary key "#(automationMix_Latest)" and store with new dictionary key "automationMix_Latest_1"
    #Comment: the user validate the ID number in History
    And the user validates Exact expected value "Compare_Strings" that the "table" element is "Equal To" "Add" at the "HistoryPage" page based on datadictionary "#(Id_Number)" and xpath1 "#(IDNumberXpath3)" and xpath2 "#(automationMix_Latest_1)" "validate_ID_Number" "HardStopOnFailure"

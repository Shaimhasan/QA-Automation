Feature: Basic UnPost Inventory Entry
  ...
  @Basic_UnPost_Inventory_Entry @RegressionSuite @BO_Inventory_Entry @Back_Office
  Scenario: Basic_UnPost_Inventory_Entry_Testcase
    ...
    And the user waits for the page to load
    #Comment: the user validate post
    And the user validates the "validatePost" element is not present at the "PostUnPostInventoryEntryPage" page "validate_Post_Value" "HardStopOnFailure"
    ...
    #Comment append value with dictionary
    And the user validates and append at trailing any value "']//preceding-sibling::td[text()='Un-Post']" with data dictionary key "#(Id_Number_Latest)" and store with new dictionary key "Id_Number_Latest_1"
    #Comment: the user validate the ID number in History
    And the user validates Exact expected value "Compare_Strings" that the "table" element is "Equal To" "Un-Post" at the "HistoryPage" page based on datadictionary "#(Id_Number)" and xpath1 "#(IDNumberXpath3)" and xpath2 "#(Id_Number_Latest_1)" "validate_ID_Number" "HardStopOnFailure"

Feature: Basic Edit Inventory Items
  This script is to validate Edit Inventory Items

  @Basic_Edit_Inventory_Items @RegressionSuite @BO_Inventory_Items @Inventory @Back_Office @Smoke_Testing
  Scenario: Basic_Edit_Inventory_Items_Testcase
    ...
    And the user clicks the "editBtn" element at the "InventoryItemsPage" page
    #Comment: the user validate the visibility of popup
    And the user waits for the "inventoryItemText" element to be "VISIBLE" on the "EditInventoryItemsPage" page
    #Comment: the user enters the name On ADD Item
    And the user enters dynamic UserName "#(name)" into the "name" textbox at the "EditInventoryItemsPage" page
    #Comment: the user store the text on data dictionary
    And store the displayed text of the "name" element at the "EditInventoryItemsPage" page into the data dictionary with key "name_value2"
    #Cooment: the user click save
    And the user clicks the "save" element at the "EditInventoryItemsPage" page
    #Comment: The user wait until page is loading
    And the user waits for the dom to load
    #Comment: the user click on the ID number row
    And the user clicks the "table" element with dictionary key "#(name_value2)" at the "InventoryItemsPage" page with xpath1 "#(IdNumberXpath1)" and xpath2 "#(IdNumberXpath2)"
    #Comment: the user store the id
    And store the displayed text of the "table" element at the "InventoryItemsPage" page and get the dictionary key value "#(name_value2)" based on xpath1 "#(IdNumberXpath1)" and xpath2 "#(IdNumberXpath2)" store at dictionary with key "Id_Number"
    #Comment: the user click on Edit Button
    And the user clicks the "editBtn" element at the "InventoryItemsPage" page
    #Comment: the user validate the visibility of popup
    And the user waits for the "inventoryItemText" element to be "VISIBLE" on the "EditInventoryItemsPage" page
    #Comment: the user store the text on data dictionary
    And store the displayed text of the "name" element at the "EditInventoryItemsPage" page into the data dictionary with key "name_value3"

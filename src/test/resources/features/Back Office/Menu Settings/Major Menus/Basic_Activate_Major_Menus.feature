Feature: Basic Activate Major Menus
  ...
  Scenario: Basic_Activate_Major_Menus_Testcase
    ...
    And the user waits for the "majorMenusTextVal" element to be "VISIBLE" on the "ActDeactMajorMenusPage" page
    #Comment: user validate the check box is selected
    And the user validates the item in the "availabelChkBoxIsSelected" checkbox is checked at the "ActDeactMajorMenusPage" page "validate_CheckBx_Selected" "HardStopOnFailure"
    #Comment: the user click on Edit Button
    And the user clicks the "cancel" element at the "ActDeactMajorMenusPage" page
    ...
    #Comment append value with dictionary
    And the user validates and append at trailing any value "']//preceding-sibling::td[text()='Activate / Deactivate']//following-sibling::td)[5]" with data dictionary key "#(menuName_value1_Latest)" and store with new dictionary key "menuName_value1_Latest_1"

Feature: Basic Add Comments
  This script is to validate Add Comments

  @Basic_Add_Comments @RegressionSuite @BO_Comments @Menu_Settings @Back_Office
  Scenario: Basic_Add_Comments_Testcase
    ...
    And the user validates the data dictionary value of "#(comments_value1)" is "Equal To" data dictionary value of "#(comments_value2)" "validate_nameUS_value" "HardStopOnFailure"
    ...
    #Comment append value with dictionary
    And the user validates and append at trailing any value "']//preceding-sibling::td[text()='Add']" with data dictionary key "#(id_value1_Latest)" and store with new dictionary key "id_value1_Latest_1"
    #Comment: the user validate the ID number in History
    And the user validates Exact expected value "Compare_Strings" that the "table" element is "Equal To" "Add" at the "HistoryPage" page based on datadictionary "#(Id_Number)" and xpath1 "#(IDNumberXpath3)" and xpath2 "#(id_value1_Latest_1)" "validate_ID_Number" "HardStopOnFailure"
